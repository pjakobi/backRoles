...
public class userController {
    ...
    private static RolesLogger rlog=new RolesLogger(userController.class.getName());
 ...
 */
 ...
 // Get user's DNs
    @GetMapping("/users")
    public ResponseEntity<ArrayList<String>> getLDAPUsers() throws FileNotFoundException, InvalidNameException, IOException, NamingException, URISyntaxException {
    	Props properties = new Props(env);
    	new RolesLogger(Level.INFO, "spif.users", new Object[] {properties.getUsersSubTree()});
    	...
    	LdapConnection cnx = new LdapConnection(uri, bindDn, pwd);
    	
    	ArrayList<String> strUsers = new ArrayList<String>();
        try {
        	LdapName searchedTree = new LdapName(properties.getUsersSubTree());
            ArrayList<LdapName> users = cnx.getUsers(searchedTree);
            for (LdapName index : users) strUsers.add(index.toString());
            rlog.doLog(Level.FINE, "spif.users.ok", new Object[] {});
    ...
    public ResponseEntity<String> test() throws NumberFormatException, IOException, InvalidNameException, NamingException, InterruptedException, OperatorCreationException, NoSuchAlgorithmException, NoSuchProviderException, InvalidPathException, JAXBException {
    	RolesLogger rlog = new RolesLogger(Level.INFO,"spif.users",new Object[] {""});
